...
## Содержание
 ...
 3. [Chapter III](#chapter-iii)      
     ...
     - [Задание 1. Создание структуры проекта](#задание-1-создание-структуру-проекта)  
...
- Выделить слой контрактов с API для взаимодействия с клиентом.
- Выделить слой приложения для реализации бизнес-логики приложения.
- Выделить слой инфраструктуры ля реализации работы с данными (например, с базой данных).
- *Выделить слой di, где описываются конфигурации для внедрения зависимостей (например, при помощи библиотеки uber-fx).
...
- Реализовать структуру-хранилище для хранения текущих игр.
- В качестве средства хранения использовать потокобезопасные коллекции (sync.Map?).
- Описать модели игрового поля, текущей игры.
- Реализовать мапперы domain<->datasource.
- Реализовать репозиторий для работы со стукртурой-хранилищем, у которого есть следующие методы:
    - Метод сохранения текущей игры.
    - Метод получения текущей игры.
- Создать структуру, реализующую интерфейс сервиса и принимающий в качестве параметра интерфейс репозитория для работы со структурой-хранилищем.
- Модели, интерфейсы, реализации должны находиться в разных файлах.

### Задание 4. Реализовать web слой
- Описать модели игрового поля, текущей игры.
- Реализовать мапперы domain<->web.
- Реализовать хендлер с использованием net/http, имеющий метод POST /game/{UUID текущей игры}, который отправляет текущую игру с обновленным игровым полем пользователем и получает в ответ текущую игру с обновленным игровым полем компьютером.
...
- Использовать библиотеку uber/fx, с помощью которой описать граф зависимостей.
